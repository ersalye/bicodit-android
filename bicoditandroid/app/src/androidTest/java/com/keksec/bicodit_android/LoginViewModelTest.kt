...
class LoginViewModelTest {
    ...
    fun createRatingViewModel() {
        ...
        loginViewModel = LoginViewModel(userDao, userApiService)
    }
    ...
    fun whenSuccessfullyLoginUserThenInsertOneToDbAndSetPrefs() {
        ...
        runBlocking {
            loginViewModel.loginUser("tester", "123456l")
    ...
    fun whenServerRespondsWithSuccessStatusThenUserLiveDataValueIsSetToSuccessEvent() {
        ...
        Handler(Looper.getMainLooper()).post {
            loginViewModel.userLiveData.observeForever {
                when (it.status) {
                    Status.ERROR -> {
    ...
    fun whenInvalidLoginProvidedThenSetLoginValidationErrorToValidatorMessage() {
        Handler(Looper.getMainLooper()).post {
            loginViewModel.loginValidationState.observeForever {
                if (it.loginError != null) {
                    assertThat(it.loginError).isEqualTo(R.string.invalid_login_characters)
                }
            }
        }
        runBlocking {
            loginViewModel.loginUser("mister test", "123456l")
        }
    }
        ...
        Handler(Looper.getMainLooper()).post {
            loginViewModel.loginValidationState.observeForever {
                if (it.passwordError != null) {
                    assertThat(it.passwordError).isEqualTo(R.string.invalid_password_characters)
                }
            }
        }
        runBlocking {
            loginViewModel.loginUser("mister", "123456")
        }
    }
        ...
        Handler(Looper.getMainLooper()).post {
            loginViewModel.userLiveData.observeForever {
                when (it.status) {
                    Status.ERROR -> {
                        assertThat(it.error)
                            .isEqualTo(Error(RequestHelpers.getLoginErrorMessage(testedStatus)))
                    }
                }
            }
        }
